...
export function LedControl() {
  ...
  const toggleLed = async (groupe: string) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        setFeedback(`LED ${groupe} → ${newState === 1 ? 'ALLUMÉE' : 'ÉTEINTE'}`)
      } else {
        setFeedback(`Erreur : ${json.error || 'inconnue'}`)
      }
    } catch {
      // Mode démo : toggle local
      setLeds(prev => ({ ...prev, [groupe]: newState }))
      setFeedback(`[DÉMO] LED ${groupe} → ${newState === 1 ? 'ON' : 'OFF'}`)
    }
  ...
  return (
    <div className="panel p-5 flex flex-col gap-4">
      ...
      {feedback && (
        <div className="px-4 py-3 text-sm bg-[#00ff41]/10 border border-[#00ff41]/30 text-[#00ff41]">
          {feedback}
        </div>
      )}
    </div>
