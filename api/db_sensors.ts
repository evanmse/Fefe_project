    ...
    if (action === 'lidar_g2d') {
      try {
        ...
        const conn = await mysql.createConnection({
          ...
          database: DB_NAME, charset: 'utf8mb4', connectTimeout: 5000,
        });
        const limit = parseInt((req.query.limit as string) || '50');
        const [rows] = await conn.execute(`SELECT * FROM G2D_LIDAR ORDER BY date_mesure DESC LIMIT ${Math.min(limit, 200)}`);
        await conn.end();
        return res.status(200).json({ success: true, data: rows, count: (rows as any[]).length, source: 'mysql' });
      } catch {
        try {
